...
setInputValue(value)
  ...
  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: (inputVale.length > 10) ? 'crimson' : 'royalblue', /* STEP 2 */
  };

  return (
    <div className='widget-input container'>
      <h2>Input</h2>
      <div id='output' style={style}>{inputVale.toUpperCase()}</div> {/* STEP 3 */}
      <div>
        <input id='input' type='text' onChange={changeInput} value={inputVale} /> {/* STEP 6 */}
